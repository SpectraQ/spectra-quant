    ...
    <div className="mx-auto max-w-2xl space-y-8">
      ...
      <div>
        ...
        <p className="mt-2 text-sm text-[var(--color-muted)]">
          Deploy a non-custodial trading vault on Solana. Your strategy runs
          inside Arcium MPC — investors see on-chain returns, your alpha stays
          private.
        </p>
      </div>
          ...
          >
            ...
            >
              ...
              <span
                ...
                style={{
                  color:
                    s.n === step ? "var(--color-text)" : "var(--color-muted)",
                }}
              >
                {s.label}
              </span>
      ...
      >
          ...
          <div className="space-y-5">
            ...
            <div className="space-y-2">
              ...
              <p className="text-[11px] text-[var(--color-muted)]">
                Specific parameters (fast/slow windows, thresholds) are
                encrypted to the Arcium MXE and never exposed on-chain.
              </p>
            </div>
          </div>
        )}
        ...
        {step === 2 && (
          <div className="space-y-5">
            ...
            <p className="text-sm text-[var(--color-muted)]">
              These values are encrypted before being stored. Only the Arcium
              MXE cluster can access them during computation — not the program,
            ...
            >
              ...
              <span className="text-xs text-[var(--color-positive)]">
                Encrypted with Arcium MXE threshold key — parameters are
            ...
            {[
              {
                ...
                hint: "Shorter lookback period (e.g. 5 candles)",
              },
              {
                key: "slowN",
                label: "Slow MA window",
                hint: "Longer lookback period (e.g. 50 candles)",
              },
              {
                key: "thresholdBps",
                label: "Signal threshold (bps)",
                hint: "Min crossover gap before triggering a trade",
              },
              {
                key: "riskPct",
                label: "Max position size (%)",
                hint: "Maximum % of vault NAV per trade",
              },
            ].map((field) => (
        ...
        {step === 3 && (
          <div className="space-y-5">
            ...
            <p className="text-sm text-[var(--color-muted)]">
              Your strategy parameters are threshold-encrypted to the Arcium MXE
              before being submitted on-chain. The MPC cluster co-computes the
              signal each tick without any node seeing the plaintext.
            </p>
            ...
            >
              {[
                [
                  "1. Encrypt",
                  "Your parameters are encrypted to the Arcium MXE threshold pubkey client-side.",
                ],
                [
                  "2. Submit",
                  "Encrypted parameters are posted on-chain via initialize_vault — never plaintext.",
                ],
                [
                  "3. Compute",
                  "Each tick, the agent calls request_signal_computation. The MXE cluster co-computes your MA crossover under MPC.",
                ],
                [
                  "4. Callback",
                  "The decrypted signal integer lands back on-chain via callback_signal. Agent executes the trade.",
                ],
              ].map(([title, body]) => (
            ...
            <div className="space-y-3">
              {[
                ...
                >
                  ...
                  <span className="mono text-xs text-[var(--color-text)]">
                    {value}
                  </span>
                </div>
              ))}
        ...
        {step === 4 && (
          <div className="space-y-5">
            ...
            >
              {[
                ...
                >
                  ...
                  <span className="mono text-xs text-[var(--color-text)] truncate max-w-xs text-right">
                    {v}
                  </span>
                </div>
              ))}
            ...
            >
              ...
              <p className="text-xs leading-relaxed text-[var(--color-muted)]">
                Deployment costs ~0.01 SOL in rent for the vault PDA, share
                mint, and two ATAs. The transaction is irreversible — your vault
                will be visible in the marketplace immediately after
                confirmation.
              </p>
            </div>
            ...
            <div className="space-y-3">
              <button
                ...
                onClick={() =>
                  alert(
                    "Multi-vault deployment coming in the next sprint — the SpectraQ Labs vault is live now on devnet.",
                  )
                }
              >
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(135deg, rgba(139,92,246,0.07), rgba(34,211,238,0.03))",
          border: "1px solid rgba(139,92,246,0.15)",
        }}
      >
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-3">
          {[
            [
              "Alpha stays private",
              "Arcium MPC encrypts your parameters end-to-end. No node, no employee, no auditor sees your signals.",
            ],
            [
              "Investors trust the code",
              "Withdrawals are enforced by the program — no manager key can block redemptions.",
            ],
            [
              "Validation framework",
              "Our four-stage MCPT pipeline helps you ship strategies with statistical rigour, not just backtests.",
            ],
          ].map(([title, body]) => (
            <div key={title}>
